    ...
    <section id="hero" className="min-h-screen pt-14 grid grid-cols-1 lg:grid-cols-2 overflow-hidden">
      ...
      <div className="relative flex flex-col justify-center overflow-hidden border-r border-white/[0.07]">
        ...
        <div className="relative z-10 px-8 lg:px-12 py-20">
          ...
          {/* eyebrow */}
          <motion.p {...up(0.15)}
            className="font-mono text-[0.6rem] tracking-[0.14em] uppercase text-amber mb-5 before:content-['$_'] before:text-[#52524e]"
          >
            {hero.eyebrow}
          </motion.p>
          ...
          <h1 className="font-serif italic leading-[1.05] tracking-[-0.015em] text-[clamp(1.9rem,3.2vw,3rem)] mb-3">
            {hero.titleLines.map((line, i) => (
              <motion.span key={i} {...up(0.28 + i * 0.11)}
                className={`block ${i > 0 ? "text-amber" : "text-[#f0ece4]"}`}
              >
                {line}
              </motion.span>
            ))}
          ...
          <motion.p {...up(0.62)} className="text-[0.875rem] text-muted leading-[1.8] mb-8 max-w-[480px]">
            {hero.bio.split(hero.boldInBio).map((part, i) =>
              i === 0
                ...
                : <span key={i}><strong className="text-[#f0ece4] font-medium">{hero.boldInBio}</strong>{part}</span>
            )}
          </motion.p>
          ...
          <motion.div {...up(0.74)} className="flex gap-3 flex-wrap">
            <a href={hero.cta1.href}
              className="font-mono text-[0.68rem] tracking-[0.1em] uppercase px-5 py-2.5 bg-amber text-[#18181b] border border-amber transition-all duration-200 hover:bg-transparent hover:text-amber hover:shadow-[0_0_20px_rgba(232,160,48,0.25)]"
            >
              {hero.cta1.label} →
            </a>
      ...
      <div className="hidden lg:flex flex-col justify-center px-10 xl:px-14 gap-5">
        ...
        <motion.div
          ...
          className="bg-[#27272a] border border-white/[0.13] rounded-sm overflow-hidden font-mono"
        >
          {/* bar */}
          <div className="bg-[#1f1f23] border-b border-white/[0.07] px-4 py-2.5 flex items-center justify-between">
          ...
          <div className="px-5 py-3 flex flex-col divide-y divide-white/[0.06]">
            {careerPath.steps.map((step, i) => (
              <div key={i} className="grid grid-cols-[20px_1fr] gap-3 py-3">
                ...
                <div>
                  <div className="text-[0.69rem] text-[#f0ece4] mb-0.5 leading-snug">
                    <span className="text-amber text-[0.62rem]">{step.company} </span>
                    {step.role}
                  </div>
                  <div className="text-[0.6rem] text-[#52524e] leading-snug">
                    {step.note}
        ...
        >
          {hero.stats.map((s) => (
            <div key={s.label}
              className="flex items-center gap-1.5 font-mono text-[0.58rem] tracking-[0.05em] px-2.5 py-1 border border-white/[0.1] text-[#52524e] rounded-sm"
            >
